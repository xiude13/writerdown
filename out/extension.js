...
function activate(context) {
    ...
    // Create providers
    const characterProvider = new characterProvider_1.CharacterProvider();
    const todoProvider = new todoProvider_1.TodoProvider();
    const structureProvider = new structureProvider_1.StructureProvider();
    // Activate test commands
    (0, basicTests_1.activateTests)(context);
    ...
    const updateWordCount = () => {
        ...
        const isWriterDownFile = activeEditor.document.languageId === 'writerdown' ||
        ...
        wordCountStatusBarItem.wordCountData = {
            words,
            characters,
            charactersNoSpaces,
            fileName: activeEditor.document.fileName.split('/').pop() || 'Unknown',
        };
        pageCountStatusBarItem.pageCountData = {
            words,
            estimatedPages,
            exactPages,
            wordsPerPage,
    ...
    const goToStructureCommand = vscode.commands.registerCommand('writerdown.goToStructure', async (structureInfo) => {
        // Open the file first
    ...
    const openCharacterCardCommand = vscode.commands.registerCommand('writerdown.openCharacterCard', async (characterInfo) => {
        await characterProvider.openCharacterCard(characterInfo);
    });
    const goToCharacterCommand = vscode.commands.registerCommand('writerdown.goToCharacter', async (characterInfo) => {
    ...
    const goToCharacterReferenceCommand = vscode.commands.registerCommand('writerdown.goToCharacterReference', async (referenceInfo) => {
        // Open the file first
    ...
    const goToTaskCommand = vscode.commands.registerCommand('writerdown.goToTask', async (todoInfo) => {
        // Open the file first
    ...
    const showWordCountDetailsCommand = vscode.commands.registerCommand('writerdown.showWordCountDetails', () => {
        const data = wordCountStatusBarItem.wordCountData;
        if (data) {
            vscode.window.showInformationMessage(`Word Count for ${data.fileName}:\n` +
                `Words: ${data.words}\n` +
                `Characters: ${data.characters}\n` +
                `Characters (no spaces): ${data.charactersNoSpaces}`, { modal: false });
        }
    });
    ...
    const showPageCountDetailsCommand = vscode.commands.registerCommand('writerdown.showPageCountDetails', () => {
        const data = pageCountStatusBarItem.pageCountData;
        if (data) {
            vscode.window.showInformationMessage(`Page Estimation for ${data.fileName}:\n` +
                `Words: ${data.words}\n` +
                `Estimated pages: ${data.estimatedPages} (rounded up)\n` +
                `Exact pages: ${data.exactPages}\n` +
                `Based on ${data.wordsPerPage} words per page (standard book format)`, { modal: false });
        }
    });
    context.subscriptions.push(setLanguageCommand, refreshStructureCommand, newChapterCommand, goToStructureCommand, refreshCharactersCommand, newCharacterCommand, openCharacterCardCommand, goToCharacterCommand, goToCharacterReferenceCommand, refreshTasksCommand, goToTaskCommand, refreshAllCommand, showWordCountDetailsCommand, showPageCountDetailsCommand, structureTreeView, characterTreeView, todoTreeView);
    ...
    const onDidOpenTextDocument = vscode.workspace.onDidOpenTextDocument(async (document) => {
        if (document.fileName.endsWith('.md') && document.languageId === 'markdown') {
            const selection = await vscode.window.showInformationMessage('This is a Markdown file. Would you like to use WriterDown syntax highlighting?', 'Use WriterDown', 'Keep Markdown');
    ...
    const onDidChangeActiveTextEditor = vscode.window.onDidChangeActiveTextEditor(async (editor) => {
        ...
        if (editor && editor.document.fileName.endsWith('.md')) {
            const hasData = structureProvider.getAllStructure().length > 0 ||
    ...
    const definitionProviderDisposable = vscode.languages.registerDefinitionProvider('writerdown', characterDefinitionProvider);
    ...
    const completionProviderDisposable = vscode.languages.registerCompletionItemProvider('writerdown', characterCompletionProvider, '@');
...
class CharacterCompletionProvider {
    ...
    provideCompletionItems(document, position, token, context) {
        ...
        const completionItems = allCharacters
            ...
            completionItem.detail = `Character (${char.count} mentions across ${new Set(char.positions.map((p) => p.fileName)).size} files)`;
    ...
    async resolveCompletionItem(item, token) {
        // Get character info
        const characterInfo = this.characterProvider.getCharacterInfo(item.label);
        if (!characterInfo || !characterInfo.cardPath) {
            return item;
        }
        try {
            ...
            item.documentation = new vscode.MarkdownString(`\`\`\`\nCharacter: ${characterInfo.name}\n` +
                `${characterInfo.count} mentions across ${new Set(characterInfo.positions.map((p) => p.fileName)).size} files.\n\n` +
                `Character card not found or couldn't be read.\n\`\`\``);
        }
        return item;
    }
    processCharacterCardForDisplay(cardContent) {
